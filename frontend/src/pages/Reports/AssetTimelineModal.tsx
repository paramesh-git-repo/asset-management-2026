...
export const AssetTimelineModal: React.FC<AssetTimelineModalProps> = ({
  isOpen,
  onClose,
  employeeName,
  assignments,
  ...
  // Create timeline events from assignments
  const timelineEvents = useMemo(() => {
    ...
    assignments.forEach((assignment) => {
      ...
      if (assignment.returnedAt) {
        const returnedAt = new Date(assignment.returnedAt);
        events.push({
  ...
  const getActivitySummary = (event: TimelineEvent): string => {
    if (event.type === 'assigned') {
      return `Asset "${event.assignment.asset.name}" assigned to ${employeeName}`;
    } else {
      return `Asset "${event.assignment.asset.name}" returned by ${employeeName}`;
    }
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Activity Timeline - ${employeeName}`} size="xl">
                              ...
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 space-y-2">
                                  ...
                                  <div className="flex items-center gap-2 text-xs text-gray-600">
                                    <span className="font-medium">{event.assignment.asset.name}</span>
                                    <span className="text-gray-400">•</span>
                                    <span>{event.assignment.asset.assetId}</span>
                                    {event.assignment.asset.category && (
                                      <>
                                        <span className="text-gray-400">•</span>
                                        <span>{event.assignment.asset.category}</span>
                                      </>
                                    )}
                                  </div>
                                  ...
                                  <div className="flex items-center gap-2 text-xs text-gray-600">
                                    ...
                                    <span className="text-gray-400">•</span>
                                    <span>{event.assignment.employee.department}</span>
                                  </div>
                                </div>
                              ...
                              <div className="border-t border-gray-200 bg-gray-50 p-4 space-y-4">
                                ...
                                {isReturned && event.assignment.remarks && (
                                  <div className="flex items-start gap-2">
                                    <User className="h-4 w-4 text-gray-400 mt-0.5 flex-shrink-0" />
                                    <div>
                                      <p className="text-xs font-medium text-gray-700">Verified By</p>
                                      <p className="text-xs text-gray-500">
                                        {event.assignment.remarks.includes('Verified by:')
                                          ? event.assignment.remarks.split('Verified by:')[1]?.split('\n')[0]?.trim() || 'N/A'
                                          : 'N/A'}
                                      </p>
                                    </div>
                                  </div>
                                    ...
                                    <div>
                                      ...
                                      <p
                                        className={`text-xs font-medium ${
                                          event.assignment.condition === 'GOOD' ? 'text-green-600' : 'text-red-600'
                                        }`}
                                      >
